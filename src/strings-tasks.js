...
function isPalindrome(str) {
  const strOnlyLetters = str.toLowerCase().replace(/[^a-zA-Z0-9]/g, '');
  const reversedStr = strOnlyLetters.split('').reverse().join('');
  return reversedStr === strOnlyLetters;
}
...
function getCardId(value) {
  ...
  return suits.indexOf(value[value.length - 1]) * 13 + ranks.indexOf(value[0]);
}
